...
function LyricLineCard({ line }: { line: JPopLyricLine }) {
  ...
  return (
    <div className={`w-full bg-surface rounded-2xl border-2 p-4 sm:p-6 transition-all duration-300 ${
        line.isChorus ? 'border-amber-500/30 bg-amber-500/5' : 
        line.isEnglish ? 'border-blue-500/30 bg-blue-500/5' : 'border-border'
      }`}>
...
export default function JPopPage() {
  const song = JPOP_SONGS[0]; // Nagareboshi - HOME MADE 家族
  ...
  return (
    <div className="flex flex-col items-center pb-24 pt-6 w-full max-w-3xl mx-auto px-4">
      ...
      {/* YouTube Player */}
      <div className="w-full aspect-video rounded-3xl overflow-hidden border-4 border-border shadow-lg mb-10 relative bg-black">
        <iframe 
      ...
      {/* Controls */}
      <div className="w-full flex flex-col sm:flex-row items-center justify-between bg-surface border-2 border-border rounded-2xl p-4 mb-8 shadow-sm gap-4">
        <div className="flex flex-col sm:flex-row items-center gap-4 text-sm font-bold text-gray-500 text-center sm:text-left">
          <p>
            Dê play no vídeo acima e acompanhe a letra lendo o <span className="text-foreground">Kanji</span>.
          </p>
        </div>
      </div>
      ...
      <div className="w-full mt-12 bg-surface border-2 border-border rounded-3xl p-6 text-center shadow-sm">
        ...
        <p className="text-gray-500 text-sm mb-4">
          Você analisou todas as {song.lyrics.length} linhas da música. Cada Kanji estudado aqui reforça sua memória e acelera a leitura.
        </p>
        <Link
          href="/"
